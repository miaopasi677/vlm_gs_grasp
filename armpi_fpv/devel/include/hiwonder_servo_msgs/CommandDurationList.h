...
struct CommandDurationList_
{
  ...
  _duration_type duration;
   ...
   typedef std::vector<uint16_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<uint16_t>> _ids_type;
  _ids_type ids;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hiwonder_servo_msgs::CommandDurationList_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.ids.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint16_t>::stream(s, true ? std::string() : indent + "    ", v.ids[i]);
    }
    if (v.ids.empty() || true)
      s << "]";
    if (true || !indent.empty())
