...
struct ServoStateList_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::hiwonder_servo_msgs::ServoState_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::hiwonder_servo_msgs::ServoState_<ContainerAllocator> >> _servo_states_type;
...
struct Printer< ::hiwonder_servo_msgs::ServoStateList_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hiwonder_servo_msgs::ServoStateList_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "servo_states: ";
    if (v.servo_states.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.servo_states.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::hiwonder_servo_msgs::ServoState_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.servo_states[i]);
    }
    if (v.servo_states.empty() || false)
      s << "]";
  }
