...
struct JointState_
{
  ...
  _name_type name;
   ...
   typedef std::vector<int32_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<int32_t>> _servo_ids_type;
  _servo_ids_type servo_ids;

   typedef std::vector<int32_t, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<int32_t>> _servo_temps_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hiwonder_servo_msgs::JointState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.servo_ids.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.servo_ids[i]);
    }
    if (v.servo_ids.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "servo_temps: ";
    if (v.servo_temps.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.servo_temps.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.servo_temps[i]);
    }
    if (v.servo_temps.empty() || true)
      s << "]";
    if (true || !indent.empty())
